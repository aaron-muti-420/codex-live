    ...
    <x-slot name="header">
        ...
        <div class="bg-white md:mx-auto rounded-lg shadow-xl w-full mt-10 md:w-1/2 overflow-hidden dark:bg-gray-800">
                ...
                <div class="h-[90px] shadow-md w-[90px] rounded-full border-4 overflow-hidden -mt-14 border-white dark:border-white">
                    ...
                    <div class="">
                        <h3 class="text-xl text-slate-900 relative font-bold leading-6 dark:text-gray-200">{{ $department->department_name}}</h3>
                        <p class="text-sm text-gray-600 dark:text-gray-400">{{$department->department_location}}</p>
                    </div>
                    ...
                    <h4 class="text-md font-medium leading-3 dark:text-gray-200">Sections</h4>
                    <div class="flex flex-grid gap-3">
                        @foreach ($department->sections as $section)
                        <a href="{{ route('sections.show', $section->id) }}" class="rounded-lg hover:bg-gray-200 dark:hover:bg-gray-700">
                        <div class="flex items-center gap-3 px-3 py-3 rounded-lg border w-full dark:border-gray-700"><svg stroke="currentColor" fill="currentColor" stroke-width="0" viewBox="0 0 576 512" class="h-8 w-8 text-slate-500 dark:text-gray-200" height="1em" width="1em" xmlns="http://www.w3.org/2000/svg">
                            ...
                            <div class="leading-3">
                                <p class=" text-sm font-bold text-slate-700 dark:text-gray-200">{{$section->section_name}}</p><span class="text-xs text-slate-600 dark:text-gray-400">{{$section->users->count()}} Employees</span>
                            </div>
                        </div>
                        </a>
                        @endforeach
